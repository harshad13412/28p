...
var computer, computerBase, computerArcher,Arrows;
var playerArrows = [];
var computerArrows = [];
...
function showArrows(index, arrows) {
  ...
  if(
    ...
    arrows[index].body.position.x > height
  ){
    if(!arrows[index].isRemoved){
      Arrows[index].remove(index , arrow)
    }
    else{
      arrow[index].trajectory = [];
    }
  }
    ...
    if (
      ...
      if(computerArcherLife <= 0){
        ...
        Matter.body.setposition(computer.body,{
          x:width  = -100,
          y:height = computer.body.position.y
        });
      }
